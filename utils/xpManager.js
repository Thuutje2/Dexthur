...
const {
  ...
  xpToLevel,
  getLevelRoles,
} = require('../commands/DeadByDaylight/utils/dbdxp');
...
async function addXp(message, userId) {
  ...
  if (newLevel > user.level) {
    ...
    const unlockedAchievements = await checkDBDLevelAchievements(
      userId,
      newLevel
    );
    ...
    if (unlockedAchievements.length > 0) {
      for (const achievementId of unlockedAchievements) {
        ...
        if (achievement) {
          targetChannel.send(
            `🏆 <@${userId}> has unlocked the achievement: **${achievement.name}**!`
          );
        }
      }
    }
    ...
    if (roleInfo) {
      ...
      if (role) {
        ...
        targetChannel.send(
          `🎉 <@${userId}> has reached **Level ${newLevel}** and received the role **${role.name}**!`
        );
      }
    }
  }
