...
async function addPoints(userId, points) {
  ...
  if (newPoints > user.points) {
    ...
    const achievementChannel = achievementChannelId
      ? user.guild.channels.cache.get(achievementChannelId)
      : null;
    const targetChannel =
      achievementChannel ||
      user.guild.channels.cache.get(user.defaultChannelId);

    // Check for Disney achievements
    const unlockedAchievements = await checkDisneyAchievements(
      userId,
      newPoints
    );
