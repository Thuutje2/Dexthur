  ...
  async execute(message, args) {
    try {
      ...
      if (userGuessData.last_correct_guess_date) {
        const correctGuessCooldown = getCooldownTime(
          userGuessData.last_correct_guess_date
        );
      ...
      // Cooldown after hitting 6 failed attempts on a character
        ...
        userGuessData.failed_attempts >= 6
      ) {
        const failedGuessCooldown = getCooldownTime(
          userGuessData.last_failed_guess_date
        );
      ...
      if (
        ...
        if (!randomCharacters || randomCharacters.length === 0) {
          return message.reply(
            'No Disney characters found in the database. Please contact an administrator.'
          );
        }
        ...
        dailyCharacter = await DisneyCharacter.findById(
          userGuessData.daily_character_id
        );
      ...
      if (!dailyCharacter) {
        return message.reply(
          'Unable to load character data. Please try again.'
        );
      }

      const dailyCharacterHints = Array.isArray(dailyCharacter.hints)
        ? dailyCharacter.hints
        : [];
      ...
      if (guessedCharacter === dailyCharacter.name.toLowerCase()) {
        ...
        try {
          const newAchievementIds = await checkDisneyAchievements(
            userId,
            newPoints
          );
          ...
          if (newAchievementIds.length > 0) {
            ...
            const achievementChannel = achievementChannelId
              ? message.guild.channels.cache.get(achievementChannelId)
              : null;
            ...
            for (const achievementId of newAchievementIds) {
              ...
              if (achievement) {
                const achievementEmbed = new EmbedBuilder()
                  ...
                  .setDescription(
                    `<@${userId}> has unlocked: **${achievement.name}**!\n*${achievement.description}*`
                  )
                  .setColor(0xffd700)
                  .addFields({
                    name: 'Rarity',
                  ...
                  .setFooter({
                    text: 'Congratulations! Keep playing to unlock more achievements!',
                  })
                  .setTimestamp();
        ...
        if (failedAttempts < 6) {
          const hintsGiven = Math.min(
            failedAttempts,
            dailyCharacterHints.length
          );
