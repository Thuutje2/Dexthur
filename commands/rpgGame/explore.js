...
module.exports = {
    ...
    async executeSlash(interaction) {
        ...
        await this.explore(user, interaction.guild.id, msg => interaction.reply({ content: msg, ephemeral: false }));
    },

    async explore(user, guildId, sendReply) {
        try {
            const player = await Player.findOne({ userId: user.id, guildId });
            if (!player) return sendReply('Je hebt nog geen karakter! Gebruik `/begin` om te starten.');
            ...
            if (player.cooldowns && player.cooldowns.explore) {
                ...
                if (timeSinceLastExplore < cooldownTime) {
                    ...
                    return sendReply(`⏰ Je moet nog **${remainingMinutes} minuten** wachten voordat je weer kunt verkennen!`);
                }
            }
