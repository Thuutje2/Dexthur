    ...
    try {
      ...
      if (matchingSurvivors.length === 0) {
        return message.channel.send(
          `No information found for survivor: ${survivorInput}`
        );
      } else if (matchingSurvivors.length > 1) {
        const survivorNames = matchingSurvivors
          .map((s) => (Array.isArray(s.name) ? s.name.join(', ') : s.name))
          .join(', ');
        return message.channel.send(
          `Multiple survivors found: ${survivorNames}. Please specify the full name.`
        );
      }
      ...
      if (!fs.existsSync(imagePath)) {
        return message.channel.send(
          `Image not found for survivor: ${Array.isArray(name) ? name.join(', ') : name}`
        );
      }
      ...
      console.error(
        'An error occurred while getting information of the survivor: ',
        error
      );
