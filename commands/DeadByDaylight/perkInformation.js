  ...
  async execute(message, args) {
    try {
      ...
      for (const survivor of survivorInformation.survivors) {
        perkData = survivor.perks.find(
          (p) => p.name.toLowerCase() === perkName
        );
      ...
      if (!perkData) {
        return message.channel.send(
          `No information found for perk: ${perkName}`
        );
      }
      ...
      console.error(
        'An error occurred while getting information of the perk: ',
        error
      );
