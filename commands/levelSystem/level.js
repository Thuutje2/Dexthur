...
const { createCanvas, loadBackground, drawAvatar } = require('../../utils/canvasUtils');
...
module.exports = {
    ...
    async createLevelCard(user, userData, rank) {
        try {
            const { canvas, ctx } = await createCanvas(800, 300);
        ...
        try {
            ...
            try {
                if (interaction.deferred) {
                    ...
                    await interaction.reply({ content: 'Error generating level card!', flags: 64 });
                }
            } catch (replyError) {
